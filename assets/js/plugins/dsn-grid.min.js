! function (t) {
	...
	gsap.registerPlugin(ScrollTrigger), u.default.registerEffect(u.animateNumber), window.dsnGrid = {}, window.$dsnEffect = {
		...
		const t = $body.find(".v-light"),
			e = $body.find(".v-dark");
		t.removeClass("v-light").addClass("v-dark"), e.addClass("v-light").removeClass("v-dark")
		...
		t.find(".dsn-animate-number").each((function () {
			...
			t.html(function (t) {
				...
				gsap.effects.animateNumber(this, {
					...
					ease: "none", // Set easing to 'none' for a linear transition
					duration: 0.5,
					onComplete: function () {
	...
	function r(t) {
		...
		return e.attr = Object(i.default)(t, "attr") || {}, Object.keys(e).length ? (e.monkeycarat_csrf = dsnParam.dsn_csrf || !1, e) : {}
	}
	e.default = function ({
		el: t,
		option: e,
	...
	async function o(t) {
		...
		})), await jQuery(t).filter('script[src*="plugins/elementor"],script[src*="plugins/woocommerce"], script[src*="plugins/monkeycarat"]').each((function () {
		...
		const e = ['meta[name="keywords"]', 'meta[name="description"]', 'meta[property^="og"]', 'meta[name^="twitter"]', "meta[itemprop]", "link[itemprop]", 'link[rel="prev"]', 'link[rel="next"]', 'link[rel="canonical"]', 'link[rel="alternate"]', 'link[rel="shortlink"]', 'link[id*="elementor"]', 'link[href*="plugins/monkeycarat"]', 'link[id*="wp-block"]', 'link[id*="google-fonts"]', 'link[id*="eael"]', 'style[id*="elementor"]', 'style[id*="eael"]', "#swiper-css"].join(","),
			n = jQuery("head"),
			i = jQuery("<head></head>").html(t.match(/<head[^>]*>([\s\S.]*)<\/head>/i)[0]),
			r = n.find(e),
			o = i.find(e);
		return await n.append(o), await jQuery(t).filter('link[href*="plugins/monkeycarat"]').each((function () {
